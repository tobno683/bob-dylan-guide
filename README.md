...
```js
{ title:"Blood on the Tracks", type:"Studio", spotify:"…",
  trackTotal:10, discs:1, mbid:"…",
  tracks:[["Tangled Up in Blue","5:42"], …],
  note:"…" }
```

Picking the right release matters more than it sounds: MusicBrainz holds dozens of editions per album, and a naive choice returns a deluxe or anniversary edition rather than the record as issued. The fetch prefers official releases, earliest year, then fewest tracks, and skips anything whose title or disambiguation mentions deluxe/anniversary/expanded/bonus/remaster. Results are then audited against the known original running orders for all forty studio albums, so a reissue tracklist gets caught rather than silently shipped.

Box sets are capped at 60 stored tracks; `trackTotal` records the real figure and the modal says it is showing a subset.

66 of the 78 records have a verified tracklist. The other 12 have no details button rather than a doubtful one — a few box sets and compilations MusicBrainz could not resolve unambiguously, plus two that were dropped for resolving to the wrong release group. Adding one by hand is just a `tracks` array on that album.
...
- **Banners.** The home page has its own hero illustration (`assets/img/dylan-header.png`); the fourteen inner pages share a second one (`assets/img/page-header.png`), carried as a slimmer band behind each page title. Both are the site's own artwork. The inner banner has an empty `alt` by design — it is decoration repeated on every page, and announcing it fourteen times would be noise for screen reader users.
